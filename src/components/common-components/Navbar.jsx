                      ...
                      <div className="flex flex-col space-y-2 px-4">
                        {item.subMenu.map((sub, i) => (
                          <Link
                            ...
                            className="text-[20px] mt-3 text-gray-800 font-medium hover:text-[#0a2879] transition"
                          >
                            {sub.name}
                          </Link>
                        ))}
                ...
                {hasSubmenu ? (
                      ...
                      <div className="flex flex-col mt-6 text-center space-y-6">
                        {item.subMenu.map((sub, i) => (
                          <Link
                            key={i}
                            to={sub.path}
                            className="text-[22px] font-medium text-gray-800 hover:text-[#0a2879]"
                            onClick={() => {
                              setIsOpen(false);
                              setOpenDropdown(null);
                            }}
                  ...
                  <Link
                    ...
                    className={`text-[22px] font-montserrat font-medium ${
                      isActive
                        ? "text-[#081F5C] border-b-4 border-[#081F5C]"
                        : "text-black hover:text-[#0a2879]"
                    }`}
                    onClick={() => setIsOpen(false)}
                  >
